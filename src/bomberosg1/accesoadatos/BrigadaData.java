...
//        return listaBrigadas;
    ...
    public List<Brigada> obtenerBrigadasPorCuartel(int idCuartel) {
        List<Brigada> brigadasAsociadas = new ArrayList<>();
        String sql = "SELECT * FROM brigada WHERE numeroCuartel = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                brigada.setEspecialidad(rs.getString("especialidad"));
                brigada.setLibre(rs.getBoolean("libre"));
                brigada.setNumeroCuartel(cuaData.verCuarteles(rs.getInt("numeroCuartel")));
                brigadasAsociadas.add(brigada);
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, " Error al acceder a la tabla brigada obtener");
        }
